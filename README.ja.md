...
### ドキュメント翻訳（レイアウト保持）
- Word（.doc/.docx）・PowerPoint（.ppt/.pptx）・Excel（.xls/.xlsx）・PDFをアップロード
- Google Cloud **Document Translation API** でドキュメント全体を翻訳。元のレイアウト・書式を保ったまま出力されるため、テキスト抽出や手動編集の工程は不要
- オンライン（同期）翻訳のみ対応：ファイルサイズ20MBまで（ネイティブPDFは300ページ、スキャンPDFは20ページまで）
- 出力形式は常にアップロード時と同じ形式（PDF→DOCX変換はDocument Translation APIのバッチ翻訳限定の機能のため、今回は非対応）
- 翻訳済みドキュメントを元の形式でそのままダウンロード

> **補足:** 上記の「OCR & 翻訳」とは異なる機能です。「OCR & 翻訳」はテキストを抽出してプレーンテキストとして扱う（手動編集が可能）のに対し、レイアウトや書式は保持されません。画像やスキャンPDFから抽出したテキストを確認・修正したい場合は「OCR & 翻訳」を、ファイル全体をそのまま翻訳したい場合は「ドキュメント翻訳」を使用してください。
...
│   │   └── 3_ドキュメント翻訳.py   # ドキュメント翻訳（レイアウト保持）ページ
│   ├── requirements.txt
│   └── Dockerfile
